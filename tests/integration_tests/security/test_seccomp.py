...
def seccompiler(tmp_path):
    ...
    class Seccompiler:
        def compile(self, data, basic=False):
            input = tmp_path / "input.json"
            input.write_text(json.dumps(data))
            bpf = tmp_path / "output.bpfmap"
            run_seccompiler_bin(bpf_path=bpf, json_path=input, basic=basic)
            return bpf
...
def test_seccomp_rust_panic(bin_seccomp_paths, seccompiler):
    ...
    fc_filters_data = json.loads(fc_filters.read_text())
