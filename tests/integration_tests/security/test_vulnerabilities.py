...
@pytest.fixture
def microvm_factory_a(microvm_factory):
    """Re-use the same factory fixture, just override the firecracker and jailer binaries"""
    revision_a = global_props.buildkite_revision_a
    workspace_dir = git_clone(Path("../build") / revision_a, revision_a)
    firecracker = get_binary("firecracker", workspace_dir=workspace_dir)
    bin_dir = firecracker.parent.resolve()

    # pylint:disable=too-few-public-methods
    class FactoryWrapper:
        """Override a MicroVMFactory's firecracker binaries"""

        def __init__(self, factory):
            self._factory = factory
            self.firecracker = bin_dir / "firecracker"
            self.jailer = bin_dir / "jailer"

        def build(self, *args, **kwargs):
            """Build a MicroVM"""
            return self._factory.build(
                *args,
                fc_binary_path=kwargs.pop("fc_binary_path", self.firecracker),
                jailer_binary_path=kwargs.pop("jailer_binary_path", self.jailer),
                **kwargs,
            )

    yield FactoryWrapper(microvm_factory)


@pytest.fixture
def uvm_any_a(
    microvm_factory_a, uvm_fun, guest_kernel, rootfs_ubuntu_22, cpu_template_any
):
    """Return uvm with revision A firecracker"""
    return uvm_fun(microvm_factory_a, guest_kernel, rootfs_ubuntu_22, cpu_template_any)
